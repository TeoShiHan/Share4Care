...
//    private lateinit var recyclerAdapter: RecyclerAdapter
    ...
    override fun onCreateView(
        ...
        eventRecyclerViewAdapter = EventRecyclerViewAdapter(arrayListOf(),this)
                ...
                if (p0.exists()) {
                    for (c in p0.children) {
                        if ((c.child("status").value as Long).toInt() == key){
                            ...
                            val likes = if(c.child("likes").exists()) c.child("likes").value as HashMap<*,*> else hashMapOf<String,String>()
                            val dislikes = if(c.child("dislikes").exists()) c.child("dislikes").value as HashMap<*,*> else hashMapOf<String,String>()
